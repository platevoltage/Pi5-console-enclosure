...
module usb2(type, loc_x, loc_y, loc_z, side, rotation, size, data, pcbsize_z, enablemask, mask) {
  ...
  if (type == "single_up_a") {
    ...
    if (enablemask == true && cmask == true && mstyle == "default") {
      ...
      if (side == "top" && rotation == 0) {
        place(
          loc_x - (size_xm - size_x) / 2, loc_y - (size_ym - size_y) / 2, loc_z + size_z - back, size_x, size_y,
          rotation, side, pcbsize_z
        ) cube([size_xm, size_ym, mlen]);
      }
      if (side == "top" && rotation == 90) {
        place(
          loc_x - .25 - (size_xm - size_x) / 2, loc_y - .25 + (size_ym - size_y) / 2, loc_z + size_z - back, size_x, size_y,
          rotation, side, pcbsize_z
        ) cube([size_xm, size_ym, mlen]);
      }
      if (side == "top" && rotation == 180) {
    ...
    if (enablemask == false) {
      place(loc_x, loc_y, loc_z, size_x, size_y, rotation, side, pcbsize_z)
        union() {
          difference() {
            color("silver") cube([size_x, size_y, size_z]);
            color("dimgray") translate([1.775, -.1, 1.8]) cube([8.45, 9, 7.78]);
          }
          color("silver") translate([2.8, -.1, 4.1]) cube([6.4, 9, 3.2]);
          for (i = [0:2:2]) {
            color("#fee5a6") translate([4.6 + i, 1.5, 7.8]) rotate([-90, 0, 0]) cube([.64, .5, 10]);
          }
          for (i = [0:2:2]) {
            color("#fee5a6") translate([4.6 + i, 1.5, 4.1]) rotate([-90, 0, 0]) cube([.64, .5, 10]);
          }
        }
    }
  }
