  ...
  if (shape == "rectangle") {
    difference() {
      ...
      translate([(width / 2), (depth / 2), (thick / 2) - adj])
        cube_fillet_inside(
          [width - gap, depth - gap, thick + (3 * adj)],
          vertical=[fillet, fillet, fillet, fillet],
          top=[0, 0, 0, 0], bottom=[0, 0, 0, 0], $fn=90
        );
      // cross ties
...
module vent(width, length, height, gap, rows, columns, orientation) {
  ...
  if (orientation == "vertical") {
    rotate([90, 0, 0])for (r = [0:length + gap:rows * (length + gap) - 1]) {
...
module vent_hex_tapered(cells_x, cells_y, thickness, cell_size, cell_spacing, orientation, z, flip) {
  xs = cell_size + cell_spacing;
  ys = xs * sqrt(3 / 4);
  rot = (orientation == "vertical") ? 90 : 0;

  rotate([rot, 0, 0]) translate([cell_size / 2, cell_size * sqrt(1 / 3) + (flip ? 6.4 : 0), z]) {
      for (ix = [0:ceil(cells_x / 2) - 1]) {
        for (iy = [0:2:cells_y - 1]) {
